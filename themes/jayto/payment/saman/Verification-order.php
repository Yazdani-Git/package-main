...
if ( $result['Success'] == 1) {
		...
		if ( $check_pay->pay_status == 0 ) {
			...
			$order_table = $wpdb->get_row( "SELECT * FROM {$table_name} WHERE refid = '{$result['TransactionDetail'][ 'RefNum' ]}'", ARRAY_A );
			$pt          = $_GET[ 'pt' ];
			if ( $pt != 'experiences' ) {
				...
				if ( $_GET[ 'pt' ] == 'hotel' ) {
					...
					$smstrta     = ' '.$tour_name.';'.$tour_date.' ';
					$smstrta2     = ' '.$tour_name.';'.$tour_date.';'.$tour_date_out.';'.$order_table['passenger_name'].';'.$order_table['passenger_famili'].' ';
					$url = home_url() . '/experiences/';
					?>
