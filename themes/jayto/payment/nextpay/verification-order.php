...
curl_setopt_array( $curl, array(
	...
	CURLOPT_POSTFIELDS     => 'api_key=' . APIKEY . '&amount=' . $results_table['amount'] . '&trans_id=' . $tr_id . '',
) );
...
if ( $result['code'] === 0 ) {
	...
	if ( $check_pay->pay_status == 0 ) {
		...
		), array( 'Authority' => $result['trans_id'] ), array(

			'%d',
			'%d',
			'%s',
		), array( '%s' ) );
		$order_table = $wpdb->get_row( "SELECT * FROM {$table_name} WHERE Authority = '{$result['trans_id']}'", ARRAY_A );
		$pt          = $_GET['pt'];
		if ( $pt != 'experiences' ) {
			...
			if ( $hoster_id['discount_price'] != '' || $hoster_id['discount_price'] != 0 ) {
				$order_table['amount'] = $hoster_id['discount_price'];
			}
