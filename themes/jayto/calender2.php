...
class Calender2 {
	...
	private function has_unavailable_dates_between($start_date, $end_date) {
		...
		while (strtotime($current_date) < strtotime($end_date)) {
			$current_date = date('Y-m-d', strtotime($current_date . ' +1 day'));
			if (strtotime($current_date) < strtotime($end_date) && !$this->is_date_available($current_date)) {
				return true;
			}
		}
	...
	public function __toString() {
		...
		$first_unavailable_after_checkin = $this->get_first_unavailable_after_checkin($this->chech_in);
		...
		for ( $i = 1; $i <= $num_days; $i ++ ) {
			...
			$is_date_reserved = $reserved && in_array( $chek_in_array, $reserved );
			...
			if (!empty($this->chech_in)) {
				...
				if (strtotime($chek_in_array) <= strtotime($this->chech_in)) {
					...
					if ($is_first_unavailable) {
						...
						$ignore = trim(str_replace(['unavailable', 'reserved'], '', $ignore));
